...
# locat = '../../data/hepth/'
# txt_emb_filename = locat + 'data_embedding.txt'
# tfidf = locat + 'hepth_tfidf.txt'
# open()
...
class _LINE(object):
    ...
    def __init__(self, graph, rep_size=128, batch_size=100, negative_ratio=5, order=3):
        ...
        cur_seed = random.getrandbits(32)  # getrandbits(k)以整型形式返回k个随机位（二进制数）。32位二进制数转为整型
        # 该函数返回一个用于初始化权重的初始化程序 "initializer"
    ...
    def build_graph(self):
        ...
        # 初始化embedding 为node_size行 rep_size列
    ...
    def batch_iter(self):
        ...
        # print(edges)  edges为重新编号过的边
        data_size = self.g.G.number_of_edges()
        edge_set = set([x[0] * numNodes + x[1] for x in edges])
        shuffle_indices = np.random.permutation(np.arange(data_size))  # 把0到417打乱
    ...
    def gen_sampling_table(self):
        table_size = 1e8
        power = 0.75
        numNodes = self.node_size  # 节点数418个
        print("Pre-procesing for non-uniform negative sampling!")  # 预处理非均匀负采样
        node_degree = np.zeros(numNodes)  # out degree
        # g.look_up_dict {'440': 0, '135': 1, '519': 2, '105': 3, '584': 4, '478': 5,...} 给每个节点一个id 从0~417
        look_up = self.g.look_up_dict  # 每个节点一个id
        for edge in self.g.G.edges():
            # look_up[edge[0]] 表示边edge的头的id  g.G[edge[0]][edge[1]]["weight"]=1
            node_degree[look_up[edge[0]]] += self.g.G[edge[0]][edge[1]]["weight"]

        # node_degree表示节点的出度 节点出度越大 越可能被选为负采样点
        norm = sum([math.pow(node_degree[i], power) for i in range(numNodes)])
        # print(norm) 278.14744672677114 所有节点度的3/4次方的和
        self.sampling_table = np.zeros(int(table_size), dtype=np.uint32)  # 采样表长度为1e8
        ...
        for j in range(numNodes):
            ...
            while i < table_size and float(i) / table_size < p:
                ...
                i += 1
        data_size = self.g.G.number_of_edges()
        self.edge_alias = np.zeros(data_size, dtype=np.int32)  # 初始化边数
        self.edge_prob = np.zeros(data_size, dtype=np.float32)  # 初始化边数
        large_block = np.zeros(data_size, dtype=np.int32)
        small_block = np.zeros(data_size, dtype=np.int32)

        # total_sum 所有边的度数和 ：299
