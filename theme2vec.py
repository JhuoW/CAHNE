...
class theme2vec:
    def __init__(self, neg_size=10, batch_size=39, epoch_num=200, walk_length=2, random_walk_epochs=5, embed_size=50,
                 sentence_len=100):
        ...
        for i in range(self.node_size):
            ...
            self.theme.append(theme_i)
        self.theme = np.array(self.theme)  # shape[1038,2,100]
        print(self.theme.shape)
    ...
    def train(self):
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                embed = [[] for _ in range(self.node_size)]
                for i in range(num_batch):
                    batch = batches[i]
                    node1, node2, node3 = zip(*batch)
                    node1, node2, node3 = np.array(node1), np.array(node2), np.array(node3)
